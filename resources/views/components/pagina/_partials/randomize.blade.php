...
<div x-data="randomize" class="bg-white py-4 px-10 my-10 rounded-xl">
    ...
    <div class="grid grid-cols-1 gap-4  md:grid-cols-2 lg:grid-cols-3">
        <template x-for="dato in randomPosts" :key="dato.id" >
            <article class="grid grid-cols-12 border rounded-md bg-slate-100 transition-all duration-500  hover:shadow-md hover:shadow-black hover:scale-[1.02]">
                ...
                <div class="flex flex-1 flex-col col-span-7 justify-between">
                    ...
                    <div class="flex justify-end gap-8 mr-2 mb-2">
                        ...
                        <a :href="`/show/${dato.id}`" class="bg-gray 100 boton-amarillo"> {{ __('See')}}</a>
                    </div>
                    </div>
                </div>
            </div>
